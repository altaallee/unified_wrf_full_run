...
rm pbs_out_wrf_hrrr
rm pbs_out_wrf_uw
rm pbs_out_wrf_postprocess_maps_conus
rm pbs_out_wrf_postprocess_maps_hrrr
rm pbs_out_wrf_postprocess_maps_uw
rm pbs_out_wrf_postprocess_soundings_conus
rm pbs_out_wrf_postprocess_soundings_hrrr
rm pbs_out_wrf_postprocess_soundings_uw
rm -rf ../gfs_ICBC_wrf
rm ../WPS/GRIBFILE*
rm ../WPS/FILE:*
rm ../WPS/geogrid.log.*
...
if [ $uw = true ]; then
    ...
    maps_uw_id=$(qsub -W depend=afterok:$wrf_uw_id -N wrf_postprocess_maps_1 -o pbs_out_wrf_postprocess_maps_uw -S /bin/bash -l select=1:ncpus=24:model=has -l walltime=1:00:00 -j oe -W group_list=s2395 -m abe -v DATE=$wrf_date,RUN=$wrf_run,HOURS=$total_hours,ENS=uw $(pwd)/run_postprocess_maps.pbs.sh)
    soundings_uw_id=$(qsub -W depend=afterok:$wrf_uw_id -N wrf_postprocess_soundings_1 -o pbs_out_wrf_postprocess_soundings_uw -S /bin/bash -l select=1:ncpus=24:model=has -l walltime=0:10:00 -j oe -W group_list=s2395 -m abe -v DATE=$wrf_date,RUN=$wrf_run,HOURS=$total_hours,ENS=uw $(pwd)/run_postprocess_soundings.pbs.sh)
fi

if [ $hrrr = true ]; then
    wrf_hrrr_id=$(qsub -W depend=afterok:$metgrid_id run_wrf_hrrr.pbs.sh)
    maps_hrrr_id=$(qsub -W depend=afterok:$wrf_hrrr_id -N wrf_postprocess_maps_1 -o pbs_out_wrf_postprocess_maps_hrrr -S /bin/bash -l select=1:ncpus=24:model=has -l walltime=1:00:00 -j oe -W group_list=s2395 -m abe -v DATE=$wrf_date,RUN=$wrf_run,HOURS=$total_hours,ENS=hrrr $(pwd)/run_postprocess_maps.pbs.sh)
    soundings_hrrr_id=$(qsub -W depend=afterok:$wrf_hrrr_id -N wrf_postprocess_soundings_1 -o pbs_out_wrf_postprocess_soundings_hrrr -S /bin/bash -l select=1:ncpus=24:model=has -l walltime=0:10:00 -j oe -W group_list=s2395 -m abe -v DATE=$wrf_date,RUN=$wrf_run,HOURS=$total_hours,ENS=hrrr $(pwd)/run_postprocess_soundings.pbs.sh)
fi

if [ $conus = true ]; then
    wrf_conus_id=$(qsub -W depend=afterok:$metgrid_id run_wrf_conus.pbs.sh)
    maps_conus_id=$(qsub -W depend=afterok:$wrf_conus_id -N wrf_postprocess_maps_1 -o pbs_out_wrf_postprocess_maps_conus -S /bin/bash -l select=1:ncpus=24:model=has -l walltime=1:00:00 -j oe -W group_list=s2395 -m abe -v DATE=$wrf_date,RUN=$wrf_run,HOURS=$total_hours,ENS=conus $(pwd)/run_postprocess_maps.pbs.sh)
    soundings_conus_id=$(qsub -W depend=afterok:$wrf_conus_id -N wrf_postprocess_soundings_1 -o pbs_out_wrf_postprocess_soundings_conus -S /bin/bash -l select=1:ncpus=24:model=has -l walltime=0:10:00 -j oe -W group_list=s2395 -m abe -v DATE=$wrf_date,RUN=$wrf_run,HOURS=$total_hours,ENS=conus $(pwd)/run_postprocess_soundings.pbs.sh)
fi
